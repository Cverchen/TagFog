...
class SupConResNetLargeScalePretrained(nn.Module):
    "backbone + projection head"
    def __init__(self, name='resnet50', dataset='ImageNet100', pretrained=True):
        super(SupConResNetLargeScalePretrained, self).__init__()
        model_fun, dim_in = model_dict[name]
        self.name = name
        self.encoder = model_fun(pretrained)
        self.addmlp = nn.Sequential(
