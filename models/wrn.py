...
# output, logits = model(input,return_feature=True)
# print(output.shape, logits.shape)

class SupCEHeadWrn(nn.Module):
    """encoder + head"""
    def __init__(self, num_class):
        super(SupCEHeadWrn, self).__init__()
        # self.encoder = WideResNet(depth=40, num_classes=num_class, widen_factor=2, dropRate=0.3)
        self.encoder = WideResNet(depth=28, num_classes=num_class, widen_factor=10, dropRate=0.3)
        dim_in = 640 # dim_in 最后一层输出的特征向量维度:
        # dim_in = 128
        
        self.addmlp = nn.Sequential(
        ...
        # self.addmlp = nn.Sequential(
        #     nn.Linear(dim_in, dim_in*8),
        #     nn.BatchNorm1d(dim_in*8),
        #     nn.ReLU(inplace=True),
        #     nn.Linear(dim_in*8, 768)
        # )
        # self.addmlp = nn.Sequential(
            # nn.Linear(dim_in, dim_in*4),
            # nn.BatchNorm1d(dim_in*4),
            # nn.ReLU(inplace=True),
            # nn.Linear(dim_in*4, dim_in*8),
            # nn.BatchNorm1d(dim_in*8),
            # nn.ReLU(inplace=True),
            # nn.Linear(dim_in*8, 768),
        # )
...
class StandardWrn(nn.Module):
    """encoder + head"""
    def __init__(self, num_class=10):
        super(StandardWrn, self).__init__()
        self.encoder = WideResNet(depth=28, num_classes=num_class, widen_factor=10, dropRate=0.3)
        dim_in = 640
