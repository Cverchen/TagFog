...
from models.resnet_big import SupConResNet,clssimp
...
def main():
    ...
    # id_train_mean = np.mean(np.concatenate(id_train_features,axis=0), axis=0)
    # print(id_train_mean.shape)
    # np.save('cifar_features_mean_3.npy', id_train_mean)
    # id_train_std = np.std(np.concatenate(id_train_features,axis=0), axis=0)
    # print(id_train_std.shape)
    # np.save('cifar_features_std_3.npy', id_train_std)
    ...
    if maha is False:
        ...
        method = 'MSP'
        result = []
        score_id = softmax_id_test[:,:num_classes].max(axis=-1)
        for i in range(len(OOD_data_list)):
            score_ood = softmax_oods[i][:,:num_classes].max(axis=-1)
            auc_ood = auc(score_id, score_ood)[0]
            fpr_ood, _ = fpr_recall(score_id, score_ood, recall)
            result.append(dict(method=method, oodset=OOD_data_list[i], auroc=auc_ood, fpr=fpr_ood))
            print(f'{method}:{OOD_data_list[i]} auroc {auc_ood:.2%}, fpr {fpr_ood:.2%}')
