...
class StandardResnet(nn.Module):
    def __init__(self, name='resnet18', dataset='cifar10'):
...
class SupConResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', dataset='cifar10', feat_dim=128):
    ...
    def forward(self, x):
        feat = self.encoder(x)
        # feat_head = self.head(feat)
        feat = F.normalize(self.head(feat), dim=1)
        return feat


class clssimp(nn.Module):
    def __init__(self, ch=512, num_classes=10):
...
class MLP(nn.Module):
    def __init__(self, dim, projection_size=512, hidden_size = 4096):
        super().__init__()
...
class Text_MLP(nn.Module):
    def __init__(self, dim, projection_size=768, hidden_size = 7680):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim, hidden_size),
            nn.BatchNorm1d(hidden_size),
